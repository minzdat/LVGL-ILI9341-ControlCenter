...
const lv_img_dsc_t * ui_imgset_temperaturev[1] = {&ui_img_temperaturev2_png};

bool switch1_state = false; 
int slider_value_mq2 = 0; // Biến lưu giá trị slider 2
int slider_value_temp = 0; // Biến lưu giá trị slider 1
...
void hours_turn_around_Animation(lv_obj_t * TargetObject, int delay)
{
    ...
    lv_anim_t PropertyAnimation_0;
    lv_anim_init(&PropertyAnimation_0);
    lv_anim_set_time(&PropertyAnimation_0, 86400000);
    lv_anim_set_user_data(&PropertyAnimation_0, PropertyAnimation_0_user_data);
    lv_anim_set_custom_exec_cb(&PropertyAnimation_0, _ui_anim_callback_set_image_angle);
    lv_anim_set_values(&PropertyAnimation_0, 0, 3600);
    lv_anim_set_path_cb(&PropertyAnimation_0, lv_anim_path_linear);
...
void bar_loading_Animation(lv_obj_t * TargetObject, int delay)
{
    ...
    lv_anim_set_values(&PropertyAnimation_0, 50, 250);
    lv_anim_set_path_cb(&PropertyAnimation_0, lv_anim_path_linear);
    lv_anim_set_delay(&PropertyAnimation_0, delay + 0);
    lv_anim_set_deleted_cb(&PropertyAnimation_0, _ui_anim_callback_free_user_data);
    lv_anim_set_playback_time(&PropertyAnimation_0, 0);
...
void ui_event_Screen1(lv_event_t * e)
{
    ...
    if(event_code == LV_EVENT_SCREEN_LOADED) {
        _ui_opacity_set(ui_Container2, 0);
        _ui_opacity_set(ui_Container3, 0);
        bar_loading_Animation(ui_Panel4, 0);
        start_screen_2_Animation(ui_Container2, 3500);
        start_screen_2_Animation(ui_Container3, 4500);
    }
}
void ui_event_Button1(lv_event_t * e)
{
...
void ui_event_Screen2(lv_event_t * e)
{
    ...
    lv_obj_t * target = lv_event_get_target(e);
    if(event_code == LV_EVENT_SCREEN_LOADED) {
        seconds_turn_around_Animation(ui_Image5, 0);
        minutes_turn_around_Animation(ui_Image6, 0);
        hours_turn_around_Animation(ui_Image7, 0);
    }
}
void ui_event_Button4(lv_event_t * e)
{
...
void ui_slider_event_cb(lv_event_t *e) {
    lv_obj_t *slider = lv_event_get_target(e); // Lấy slider đang tương tác
    lv_obj_t *label = (lv_obj_t *)lv_event_get_user_data(e); // Lấy label gắn kèm
    int32_t value = lv_slider_get_value(slider); // Lấy giá trị slider
    lv_label_set_text_fmt(label, "%d", value); // Cập nhật text
    // lv_obj_align_to(label, slider, LV_ALIGN_OUT_RIGHT_MID, 10, 0); // Canh chỉnh label

     // Xác định slider và lưu giá trị tương ứng
    if (slider == ui_Slider1) { // Kiểm tra nếu slider là ui_Slider1
        slider_value_temp = value; // Lưu giá trị vào slider_value_temp
    } else if (slider == ui_Slider2) { // Kiểm tra nếu slider là ui_Slider2
        slider_value_mq2 = value; // Lưu giá trị vào slider_value_mq2
    }

    printf("slider_value_mq2: %d, slider_value_temp: %d\n", slider_value_mq2, slider_value_temp); 
}

void switch_event_handler(lv_event_t *e) {
    lv_obj_t *switch_obj = lv_event_get_target(e); // Lấy đối tượng switch
...
// Hàm tính toán và cập nhật góc quay dựa vào giờ, phút, giây
void update_clock(lv_obj_t *hour_img, lv_obj_t *minute_img, lv_obj_t *second_img, int hour, int minute, int second) {
    // Tính góc cho từng kim
    int hour_angle = (hour % 12) * 30 + (minute * 0.5); // Giờ tính theo 12 giờ
    int minute_angle = minute * 6 + (second * 0.1);
    int second_angle = second * 6;

    // Cập nhật góc quay
    lv_img_set_angle(hour_img, hour_angle * 10); // lv_img_set_angle nhận giá trị góc * 10
    lv_img_set_angle(minute_img, minute_angle * 10);
    lv_img_set_angle(second_img, second_angle * 10);
}
///////////////////// SCREENS ////////////////////
